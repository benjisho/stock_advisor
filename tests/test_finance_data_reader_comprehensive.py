#!/usr/bin/env python3
"""
Comprehensive test suite for FinanceDataReader functionality.
This file consolidates all the individual FDR test files into a single comprehensive suite.
"""
import pytest
import pandas as pd
...
class TestFinanceDataReaderEdgeCases:
    ...
    def test_comprehensive_edge_cases(self):
        """Test comprehensive edge cases including crypto and indices"""
        test_cases = [
            ("AAPL", "Valid large cap stock", True),
            ("BTC-USD", "Cryptocurrency", False),  # May or may not work
            ("^GSPC", "Index (S&P 500)", False),   # May or may not work
        ]
        
        import FinanceDataReader as fdr
        
        for symbol, description, should_work in test_cases:
            try:
                data = fdr.DataReader(symbol, '2020')
                ...
                if not data.empty:
                    ...
                    print(f"✅ {symbol} ({description}): {len(data)} rows")
                else:
                    print(f"❌ {symbol} ({description}): No data returned")
                    
            except Exception as e:
                if should_work:
                    print(f"⚠️ {symbol} ({description}): Error - {e}")
                else:
                    print(f"❌ {symbol} ({description}): Expected error - {e}")
        ...
        for symbol in valid_symbols[:2]:  # Test first 2 symbols to keep test time reasonable
            ...
            try:
                ...
                if not data.empty:
                    ...
                    print(f"Fallback simulation for {symbol}: {len(data)} rows, latest price: ${current_price:.2f}")
...
class TestFinanceDataReaderIntegration:
    ...
    def test_complete_data_workflow(self):
        ...
        try:
            ...
            if not data.empty:
                ...
                print(f"Complete workflow test: {len(data)} rows, current price: ${current_price}")
    ...
    def test_smoke_basic_functionality(self):
        ...
        try:
            ...
            print(f"Smoke test passed: {len(data)} rows of data retrieved")
